...
func (w *LocalWarden) IsAllowed(ctx context.Context, a *firewall.AccessRequest) error {
	statsdResource := x.SanitizeForStatsd(a.Resource, "_")
	...
	if err := w.isAllowed(ctx, &ladon.Request{
		...
		w.R.Logger().WithFields(logrus.Fields{
			"subject": a.Subject,
			"request": a,
...
func (w *LocalWarden) TokenAllowed(ctx context.Context, token string, a *firewall.TokenAccessRequest, scopes ...string) (*firewall.Context, error) {
	c := w.newBasicContext()
	statsdResource := x.SanitizeForStatsd(a.Resource, "_")
	statsdAction := x.SanitizeForStatsd(a.Action, "_")

	_, auth, err := w.R.OAuth2Provider().IntrospectToken(ctx, token, fosite.AccessToken, oauth2.NewSession(""), scopes...)
	if err != nil {
		w.R.Logger().WithFields(logrus.Fields{
			"request": a,
...
func (w *LocalWarden) isAllowed(ctx context.Context, a *ladon.Request) error {
	groups, err := w.R.GroupManager().FindGroupNames(a.Subject)
	if err != nil {
		return err
	}
